    ...
    <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-24 pb-20 sm:pb-24 lg:pb-32">
      ...
      <div className="text-center mb-16 sm:mb-20">
        <h1 className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-gray-900 leading-[1.1] sm:leading-tight mb-4 sm:mb-6 w-full break-words">
          <span className="bg-gradient-to-tl from-brand-dark via-brand-medium to-brand-light bg-clip-text text-transparent">Nosotros</span>
        </h1>
        <p className="text-lg sm:text-xl md:text-2xl text-gray-600 leading-relaxed max-w-4xl mx-auto mb-12">
          Somos el partner estratégico que acelera el crecimiento de empresas mexicanas con visión global
        </p>
        
        {/* Stats */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 bg-gradient-to-tl from-white via-brand-pale to-brand-light rounded-2xl p-8 sm:p-12">
          {stats.map((stat, index) => (
            <div key={index} className="text-center">
              <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-brand-dark mb-2">{stat.value}</div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-8 sm:p-12 shadow-lg">
            <div className="text-center">
              <div className="w-20 h-20 bg-gradient-to-tl from-brand-dark via-brand-medium to-brand-light rounded-full flex items-center justify-center text-white text-3xl font-bold mb-6 mx-auto">
                8
              </div>
              <h3 className="text-2xl font-bold text-gray-900 mb-4">Años de experiencia</h3>
              <p className="text-gray-600 leading-relaxed">transformando el panorama empresarial mexicano</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-16 sm:mb-20">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {values.map((value, index) => (
            <div key={index} className="bg-white rounded-2xl border border-gray-200 p-6 sm:p-8 hover:shadow-lg transition-all duration-300">
      ...
      <div className="mb-16 sm:mb-20">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {team.map((member, index) => (
            <div key={index} className="bg-white rounded-2xl border border-gray-200 p-6 sm:p-8 text-center hover:shadow-lg transition-all duration-300">
              <div className="w-24 h-24 bg-gradient-to-tl from-brand-dark via-brand-medium to-brand-light rounded-full flex items-center justify-center text-white text-2xl font-bold mb-6 mx-auto">
                {member.name.split(' ').map(n => n[0]).join('')}
              </div>
              <h3 className="text-xl font-bold text-gray-900 mb-2">{member.name}</h3>
              <p className="text-brand-medium font-semibold mb-3">{member.role}</p>
              <p className="text-gray-600 mb-2">{member.experience}</p>
              <p className="text-sm text-gray-500 font-medium">{member.specialty}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Call to Action */}
      <div className="bg-gradient-to-tl from-brand-dark via-brand-medium to-brand-light rounded-2xl p-8 sm:p-12 text-center text-white">
